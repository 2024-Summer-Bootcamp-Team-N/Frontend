...
const HousePage = () => {
  ...
  const handleShortTermRentalClick = () => {
    setIsShortTermRentalActive(!isShortTermRentalActive);
  };

  const handleParkingAvailableClick = () => {
    setIsParkingAvailableActive(!isParkingAvailableActive);
  };
    ...
    <div className="flex flex-col min-h-screen w-full h-full overflow-hidden bg-white">
      ...
      <div className="flex relative w-full h-[61px] justify-start items-center font-[NanumSquareRoundB] text-[16px] text-black">
        ...
        <button className="relative flex mr-[10px]" onClick={handleShortTermRentalClick}>
          <img src={isShortTermRentalActive ? ActiveOption : InactiveOption} alt="단기임대" />
          <p
